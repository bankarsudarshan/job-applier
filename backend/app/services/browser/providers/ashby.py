...
class AshbyProvider(ATSProvider):
    async def detect(self) -> bool:
        ...
        if "ashbyhq.com" in url:
            return True
        form = await self.page.query_selector("[class*='ashby']")
    ...
    async def extract_form(self) -> list[dict[str, Any]]:
        ...
        for inp in inputs:
            ...
            label_text = name
            parent = await inp.evaluate_handle("el => el.closest('.form-field, .input-container') || el.parentElement")
            if parent:
    ...
    async def fill_and_submit(self, fields: list[dict[str, Any]], resume_path: str | None = None) -> bool:
        try:
            ...
            logger.info("Ashby form filled successfully (submit bypassed).")
            return True
