...
class SmartRecruitersProvider(ATSProvider):
    async def detect(self) -> bool:
        ...
        if "smartrecruiters.com" in url:
            return True
        form = await self.page.query_selector("[class*='smartrecruiters']")
    ...
    async def extract_form(self) -> list[dict[str, Any]]:
        ...
        for inp in inputs:
            ...
            label_text = name
            parent = await inp.evaluate_handle("el => el.closest('.form-group, .field') || el.parentElement")
            if parent:
    ...
    async def fill_and_submit(self, fields: list[dict[str, Any]], resume_path: str | None = None) -> bool:
        try:
            ...
            logger.info("SmartRecruiters form filled successfully (submit bypassed).")
            return True
